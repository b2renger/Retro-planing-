...
export const HardwareMediaView: React.FC = () => {
  ...
  return (
    <div className="space-y-4 max-w-7xl mx-auto pb-12">
      ...
      <div className="bg-[#0D121F] border border-white/10 rounded-2xl p-4 sm:p-5 shadow-xl">
        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
          <div className="space-y-1.5 min-w-0">
            <div className="flex items-center gap-2 flex-wrap">
              ...
              <span className="px-2.5 py-0.5 rounded-full text-[10px] font-medium bg-blue-500/10 text-blue-300 border border-blue-500/20">
          ...
          <div className="flex items-center gap-2 shrink-0 flex-wrap">
            <button
              ...
              className="px-3 py-2 rounded-xl bg-purple-600/20 hover:bg-purple-600/30 border border-purple-500/30 text-purple-200 text-xs font-semibold flex items-center gap-1.5 transition-colors"
            >
            ...
            <button
              ...
              className="px-3 py-2 rounded-xl bg-blue-600 hover:bg-blue-500 text-white text-xs font-semibold flex items-center gap-1.5 shadow-md shadow-blue-600/20 transition-all"
            >
      ...
      <div className="flex items-center gap-1.5 border-b border-white/10 pb-2 overflow-x-auto">
        ...
        <button
          ...
          className={`px-3.5 py-2 rounded-xl text-xs font-semibold flex items-center gap-2 transition-all shrink-0 ${
            activeSubTab === 'testing'
              ? 'bg-pink-600/20 text-pink-200 border border-pink-500/40 shadow-sm'
              : 'text-slate-400 hover:text-slate-200 hover:bg-white/5'
          }`}
        >
        ...
        <button
          ...
          className={`px-3.5 py-2 rounded-xl text-xs font-semibold flex items-center gap-2 transition-all shrink-0 ${
            activeSubTab === 'review'
              ? 'bg-amber-600/20 text-amber-200 border border-amber-500/40 shadow-sm'
              : 'text-slate-400 hover:text-slate-200 hover:bg-white/5'
          }`}
        >
        ...
        <div className="space-y-3 animate-fadeIn">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            {filteredHardware.map((item) => (
              <div
                key={item.id}
                className="bg-[#0D121F] border border-white/10 hover:border-white/20 rounded-xl p-4 transition-all space-y-2.5"
              >
                  ...
                  <div className="flex items-start gap-2.5">
                    ...
                    <div>
                      ...
                      <div className="text-[11px] text-slate-400 flex items-center gap-2 mt-0.5">
                        <span className="font-mono text-purple-300">Qty: {item.quantity}</span>
                        <span>&bull;</span>
                ...
                <div className="bg-[#080C14] p-2.5 rounded-lg text-xs text-slate-300 font-mono text-[11px] border border-white/5">
                  {item.specs}
                </div>
                ...
                {item.vendor && (
                  <div className="flex items-center justify-between text-[11px] text-slate-400 pt-1 border-t border-white/5">
                    ...
                    {item.notes && <span className="truncate max-w-[240px] text-slate-500 italic">{item.notes}</span>}
                  </div>
                )}
              </div>
            ))}
        ...
        <div className="space-y-3 animate-fadeIn">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            {mediaList.map((asset) => (
              <div
                key={asset.id}
                className="bg-[#0D121F] border border-white/10 hover:border-white/20 rounded-xl p-4 transition-all space-y-3"
              >
                <div className="flex items-start justify-between gap-2">
                  <div className="flex items-start gap-2.5">
                    <div
                    ...
                    <div>
                      <h4 className="text-sm font-semibold text-slate-100">{asset.title}</h4>
                      <div className="text-[11px] text-slate-400 flex items-center gap-2 mt-0.5">
                        <span className="capitalize font-medium text-slate-300">{asset.type} Track</span>
                        <span>&bull;</span>
                        <span className="font-mono text-purple-300">{asset.duration}</span>
                      </div>
                    </div>
                  </div>
                ...
                <div className="bg-[#080C14] p-2.5 rounded-lg text-[11px] text-slate-300 font-mono border border-white/5">
                  Format: {asset.format}
                </div>
      ...
      {activeSubTab === 'booking' && (
        <div className="space-y-3 animate-fadeIn">
          <div className="p-3 bg-blue-950/20 border border-blue-500/30 rounded-xl text-xs text-blue-300 flex items-center justify-between">
            <div className="flex items-center gap-2">
              <Calendar className="w-4 h-4 text-blue-400" />
              <span>Phase 1: Booking & Procurement Window (Sept 15 &rarr; Oct 05, 2026)</span>
            </div>
            <span className="font-semibold text-blue-200">Status: Completed ✅</span>
          </div>

          <div className="space-y-2">
            {bookingTasks.map((t) => (
              <div key={t.id} className="bg-[#0D121F] border border-white/10 rounded-xl p-3.5 space-y-2">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-1.5 pt-1">
                  {t.checklist.map((item) => (
                    <div key={item.id} className="flex items-center gap-2 text-[11px] text-slate-300 bg-white/[0.02] p-1.5 rounded-lg">
      ...
      {activeSubTab === 'testing' && (
        <div className="space-y-3 animate-fadeIn">
          <div className="p-3 bg-pink-950/20 border border-pink-500/30 rounded-xl text-xs text-pink-300 flex items-center justify-between">
            <div className="flex items-center gap-2">
              <Zap className="w-4 h-4 text-pink-400" />
              <span>Phase 3: On-Site Testing & Calibration Window (Oct 29 &rarr; Nov 12, 2026)</span>
            </div>
            <span className="font-semibold text-pink-200">12 Checkpoint Verification To-Dos</span>
          </div>

          <div className="space-y-3">
            {testingTasks.map((t) => (
              <div key={t.id} className="bg-[#0D121F] border border-white/10 rounded-xl p-4 space-y-3">
                <div className="flex items-center justify-between gap-2">
                  <div>
                    <h4 className="font-semibold text-sm text-slate-100">{t.title}</h4>
                    <p className="text-xs text-slate-400 mt-0.5">{t.description}</p>
                  </div>
                  <span className="px-2.5 py-1 rounded-full text-xs font-semibold bg-amber-500/10 text-amber-400 border border-amber-500/20 shrink-0">
                    {t.estimatedHours}h allocated
                  </span>
                </div>

                <div className="space-y-1.5 bg-[#080C14] p-3 rounded-xl border border-white/5">
                  <div className="text-[10px] font-semibold text-slate-400 uppercase tracking-wider mb-2">
                    To-Do Calibration Checkpoints
                  </div>
                  {t.checklist.map((c) => (
                    <div key={c.id} className="flex items-start gap-2.5 text-xs text-slate-200">
                      <div className="w-4 h-4 rounded border border-white/20 flex items-center justify-center shrink-0 mt-0.5 bg-white/5">
                        {c.completed && <Check className="w-3 h-3 text-emerald-400" />}
                      </div>
                      <span>{c.text}</span>
                    </div>
                  ))}
      ...
      {activeSubTab === 'review' && (
        <div className="space-y-3 animate-fadeIn">
          <div className="p-3 bg-emerald-950/20 border border-emerald-500/30 rounded-xl text-xs text-emerald-300 flex items-center justify-between">
            ...
            <span className="font-semibold text-emerald-200">Opening Night Target</span>
          </div>

          <div className="space-y-3">
            {reviewTasks.map((t) => (
              <div key={t.id} className="bg-[#0D121F] border border-white/10 rounded-xl p-4 space-y-2">
                <div className="flex items-center justify-between">
                  <h4 className="font-semibold text-sm text-slate-100">{t.title}</h4>
                  <span className="text-xs text-emerald-400 font-mono">Due: {t.dueDate}</span>
                </div>
                <p className="text-xs text-slate-400">{t.description}</p>
                <div className="space-y-1.5 pt-2">
                  {t.checklist.map((c) => (
                    <div key={c.id} className="flex items-center gap-2 text-xs text-slate-300">
                      <div className="w-3.5 h-3.5 rounded border border-white/20 flex items-center justify-center bg-white/5 shrink-0" />
                      <span>{c.text}</span>
                    </div>
                  ))}
                </div>

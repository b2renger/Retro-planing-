...
export const TeamCollaborationView: React.FC = () => {
  ...
  return (
    <div className="w-full max-w-7xl mx-auto p-3 sm:p-6 space-y-4 sm:space-y-6 overflow-hidden">
      {/* Top Banner */}
      <div className="bg-[#0D121F] border border-white/10 rounded-2xl p-4 sm:p-5 shadow-xl bg-gradient-to-r from-purple-950/30 via-slate-900 to-transparent">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-2">
            ...
            <div className="flex items-center gap-1.5 bg-white/[0.03] px-3 py-1.5 rounded-xl border border-white/10">
              <span className="flex h-2 w-2 relative">
                <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-emerald-400 opacity-75"></span>
                <span className="relative inline-flex rounded-full h-2 w-2 bg-emerald-500"></span>
              </span>
              <span className="text-xs text-emerald-400 font-semibold">Live Storage Active</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div className="lg:col-span-7 space-y-4">
          ...
          <div className="bg-[#0D121F] border border-white/10 rounded-2xl p-4 sm:p-5 space-y-4">
            <div className="flex items-center justify-between">
              ...
              <span className="text-[11px] text-slate-400">Current persona: <strong className="text-purple-300">{currentUser.name}</strong></span>
            </div>
            ...
            <div className="space-y-3">
              {userWorkloads.map(({ user, taskCount, totalHours, completedTasks }) => {
                ...
                return (
                  <div
                    key={user.id}
                    className="bg-white/[0.02] p-4 rounded-xl border border-white/10 hover:border-white/20 transition-all space-y-2.5"
                  >
                    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
                      ...
                      <div className="flex items-center gap-2 self-end sm:self-auto">
                        <div className="text-right pr-2">
                          <span className="text-xs font-mono font-bold text-purple-300">{totalHours} hrs</span>
                          <div className="text-[10px] text-slate-400">{taskCount} tasks assigned</div>
                        </div>

                        {!isCurrent && (
                          <button
                            onClick={() => setCurrentUser(user)}
                            className="p-1.5 rounded-lg bg-blue-500/10 hover:bg-blue-500/20 text-blue-300 border border-blue-500/20 text-[10px] font-semibold flex items-center gap-1 transition-colors"
          ...
          {invitations.length > 0 && (
            <div className="bg-[#0D121F] border border-white/10 rounded-2xl p-4 sm:p-5 space-y-3">
              <div className="flex items-center justify-between">
              ...
              <div className="space-y-2">
                {invitations.map((inv) => (
                  <div
                    key={inv.id}
                    className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 p-3 rounded-xl bg-white/[0.02] border border-white/5"
                  >
                    <div>
                      <div className="flex items-center gap-2">
                    ...
                    <div className="flex items-center gap-1.5 shrink-0">
                      ...
                      >
                        {copiedId === inv.id ? <Check className="w-3 h-3 text-emerald-400" /> : <Copy className="w-3 h-3" />}
        ...
        <div className="lg:col-span-5 space-y-4">
          <div className="bg-[#0D121F] border border-white/10 rounded-2xl p-4 sm:p-5 flex flex-col h-[560px] shadow-xl">
            ...
            {/* Comments Stream */}
            <div className="flex-1 overflow-y-auto py-3 space-y-3 divide-y divide-white/5">
            ...
            {/* Post Comment Input */}
            <form onSubmit={handlePostComment} className="pt-3 border-t border-white/10 flex gap-2">
              <input
                type="text"
                placeholder="Leave design feedback, @mention teammates..."
                value={commentText}
                onChange={(e) => setCommentText(e.target.value)}
                className="flex-1 px-3.5 py-2 rounded-xl bg-[#131927] border border-white/10 text-xs text-slate-100 placeholder-slate-500 focus:outline-none focus:border-purple-500"
              />
              <button
                type="submit"

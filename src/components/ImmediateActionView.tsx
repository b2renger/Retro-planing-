...
export const ImmediateActionView: React.FC = () => {
  ...
  return (
    <div className="w-full max-w-7xl mx-auto px-3 sm:px-6 py-4 space-y-4 sm:space-y-6 overflow-hidden">
      ...
      <div className="bg-[#0D121F] border border-white/10 rounded-2xl p-4 sm:p-5 shadow-lg relative overflow-hidden">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-3 relative z-10">
          <div className="space-y-1 min-w-0">
            <div className="flex items-center gap-2">
              ...
              <span className="text-[10px] sm:text-[11px] font-semibold uppercase tracking-wider text-purple-400">
                Design Operations Triage & Queue
              </span>
            </div>
            <h2 className="text-base sm:text-lg font-semibold text-slate-100 tracking-tight truncate">
              Immediate Deliverables & Critical Path Focus
            </h2>
            <p className="text-xs text-slate-400 leading-relaxed max-w-3xl">
              Target launch <span className="text-slate-200 font-semibold font-mono">{activeProject.targetDeliveryDate}</span>. Prioritizing{' '}
              <span className="text-rose-400 font-semibold">{criticalPathTasks.length} critical path deliverables</span> to safeguard delivery runway.
            </p>
          </div>
          ...
          <div className="flex items-center gap-2 shrink-0">
            <button
              ...
              className="flex items-center gap-2 px-3.5 py-2 rounded-xl bg-emerald-500/10 hover:bg-emerald-500/20 border border-emerald-500/30 text-emerald-300 text-xs font-semibold transition-all cursor-pointer"
        ...
        {aiAuditReport && (
          <div className="mt-3.5 p-3.5 bg-purple-950/30 border border-purple-800/40 rounded-xl text-xs text-purple-200 flex items-start justify-between gap-3 animate-fadeIn">
            ...
            <button
              ...
              className="text-purple-400 hover:text-purple-200 text-xs font-bold px-2 py-0.5 shrink-0"
            >
              Dismiss
            </button>
          </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-4 sm:gap-6 min-w-0">
        ...
        <div className="lg:col-span-7 space-y-4 min-w-0">
          <div className="bg-[#0D121F] border border-white/10 rounded-2xl p-4 sm:p-5 space-y-3.5 min-w-0">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2.5 pb-3 border-b border-white/5">
              ...
              <button
                ...
                className="text-xs font-semibold text-purple-400 hover:text-purple-300 flex items-center gap-1 shrink-0 self-start sm:self-auto"
              >
              ...
              {displayedTasks.length === 0 ? (
                ...
                displayedTasks.map((task) => {
                  ...
                  return (
                    <div
                      key={task.id}
                      className="bg-[#141B2D]/40 hover:bg-[#141B2D]/70 border border-white/5 hover:border-white/10 rounded-xl p-3 sm:p-3.5 transition-all space-y-2.5 min-w-0"
                    >
                      ...
                      <div className="flex items-start gap-2.5 min-w-0">
                        ...
                        <button
                          ...
                          className={`mt-0.5 w-4 h-4 rounded border flex items-center justify-center transition-colors shrink-0 ${
                            task.status === 'done'
                              ...
                              : 'border-slate-600 hover:border-purple-400 text-transparent hover:text-purple-400'
                          }`}
                        ...
                        <div className="space-y-1 flex-1 min-w-0">
                          <div className="flex items-center gap-1.5 flex-wrap">
                            <span className="text-xs font-semibold text-slate-100 hover:text-purple-300 transition-colors break-words">
                              {task.title}
                            </span>
                            ...
                            {task.isCriticalPath && (
                              <span className="text-[9px] font-semibold px-1.5 py-0.2 rounded bg-rose-500/20 text-rose-300 border border-rose-500/30 flex items-center gap-0.5 shrink-0">
                              ...
                              <div className="grid grid-cols-1 sm:grid-cols-2 gap-1">
                                {task.checklist.map((item) => (
                                  <label
                                    ...
                                    className="flex items-center gap-1.5 text-[10px] text-slate-300 hover:text-slate-100 cursor-pointer select-none bg-black/20 px-2 py-1 rounded truncate"
                                  >
                                    <input
                                      type="checkbox"
                                      checked={item.completed}
                                      readOnly
                                      className="rounded border-slate-700 text-purple-500 focus:ring-0 w-3 h-3 shrink-0"
                                    />
                          ...
                          <div className="flex items-center justify-between pt-1 text-[10px] text-slate-400 flex-wrap gap-2">
                            <div className="flex items-center gap-2 shrink-0">
                              <img
                                src={assignee.avatar}
                                alt={assignee.name}
                              ...
                              <span>&bull;</span>
                              <span className="text-amber-400 font-medium flex items-center gap-1 font-mono shrink-0">
                                <Clock className="w-3 h-3" />
                                Due {task.dueDate}
                              </span>
                              <span>&bull;</span>
                              <span className="font-mono text-slate-400 shrink-0">{task.estimatedHours}h</span>
                            </div>
                            ...
                            <div className="flex items-center gap-1 flex-wrap">
                              {task.deliverables.map((deliv, idx) => (
                                <span
                                  key={idx}
                                  className="text-[9px] font-mono px-1.5 py-0.2 rounded bg-purple-950/40 text-purple-300 border border-purple-500/20 truncate max-w-[140px]"
                                >
                                  {deliv}
                                </span>
                              ))}
        ...
        <div className="lg:col-span-5 space-y-4 min-w-0">
          ...
          <div className="bg-[#0D121F] border border-purple-500/20 rounded-2xl p-4 sm:p-5 space-y-3 min-w-0">
            <div className="flex items-center justify-between pb-2.5 border-b border-white/5">
              ...
              <span className="text-[9px] font-semibold px-2 py-0.2 rounded-full bg-purple-500/20 text-purple-300 shrink-0">
                Gemini Agent
              </span>
            </div>
                ...
                <div
                  ...
                  className={`p-3 rounded-xl border transition-all min-w-0 ${
                    q.resolved
                      ...
                      : 'bg-purple-950/20 border-purple-700/40 shadow-sm'
                  }`}
                >
                  <div className="flex items-start gap-2 min-w-0">
                    <HelpCircle
                      className={`w-3.5 h-3.5 shrink-0 mt-0.5 ${q.resolved ? 'text-slate-500' : 'text-purple-400'}`}
                    />
                    <div className="space-y-1.5 flex-1 min-w-0">
                      <div className="text-xs font-semibold text-slate-100 break-words">{q.question}</div>
                      <div className="text-[10px] text-purple-300/80 leading-relaxed">{q.reason}</div>
                      ...
                      {q.resolved ? (
                        ...
                        <div className="pt-1.5 space-y-1">
                          <div className="text-[9px] font-semibold text-slate-400">Choose decision to apply:</div>
                          <div className="flex flex-wrap gap-1">
                            {q.suggestedOptions.map((opt, idx) => (
                              <button
                                key={idx}
                                onClick={() => resolveClarification(q.id, opt)}
                                className="text-[9px] px-2 py-0.5 rounded-lg bg-purple-900/40 hover:bg-purple-800/60 border border-purple-600/40 text-purple-200 font-medium transition-colors text-left truncate max-w-full"
                              >
                                {opt}
                              </button>
                            ))}
          ...
          <div className="bg-[#0D121F] border border-white/10 rounded-2xl p-4 sm:p-5 space-y-3 min-w-0">
            <div className="flex items-center justify-between pb-2.5 border-b border-white/5">
              <div className="flex items-center gap-2">
            ...
            <div className="relative pl-4 space-y-2.5 before:absolute before:left-1.5 before:top-2 before:bottom-2 before:w-px before:bg-white/10">
                ...
                <div key={ms.id} className="relative min-w-0">
                  <div
                    className={`absolute -left-4 top-1 w-2.5 h-2.5 rounded-full border-2 ${
                      ms.completed
                        ? 'bg-emerald-500 border-[#0D121F]'
                        : index === 0 || !activeProject.milestones[index - 1]?.completed
                        ? 'bg-amber-500 border-[#0D121F] ring-1 ring-amber-500/30'
                        : 'bg-slate-700 border-[#0D121F]'
                    }`}
                  />

                  <div className="bg-[#141B2D]/40 p-2.5 rounded-xl border border-white/5 min-w-0">
                    <div className="flex items-center justify-between gap-1">
                      <span className="text-xs font-semibold text-slate-100 truncate">{ms.title}</span>
                      <span className="text-[10px] font-semibold text-amber-400 font-mono shrink-0">
                        {ms.targetDate}
                      </span>
                    </div>
                    <p className="text-[10px] text-slate-400 mt-0.5 leading-relaxed line-clamp-1">{ms.description}</p>
                  </div>
                </div>
              ))}
            </div>
          ...
          <div className="bg-[#0D121F] border border-white/10 rounded-2xl p-4 text-center">
            <div className="w-8 h-8 rounded-xl bg-purple-500/10 border border-purple-500/20 text-purple-400 flex items-center justify-center mx-auto mb-2">
              <FileText className="w-4 h-4" />
            </div>
            <h4 className="text-xs font-semibold text-slate-100 uppercase tracking-wider">Markdown & Notes Studio</h4>
            ...
            <button
              ...
              className="mt-2.5 inline-flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-white/5 hover:bg-white/10 border border-white/10 text-slate-200 text-xs font-semibold transition-colors cursor-pointer"
            >

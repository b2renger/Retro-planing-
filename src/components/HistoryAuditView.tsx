...
export const HistoryAuditView: React.FC = () => {
  ...
  const getActionBadge = (type: HistoryEntry['actionType']) => {
    switch (type) {
      ...
      case 'status_change':
        return (
          <span className="px-2 py-0.5 rounded-full bg-blue-500/20 text-blue-700 dark:text-blue-300 border border-blue-500/30 text-[10px] font-bold flex items-center gap-1">
      ...
      case 'update':
        return (
          <span className="px-2 py-0.5 rounded-full bg-line-strong text-fg text-[10px] font-bold">
            Updated
          </span>
        );
      case 'delete':
      ...
      default:
        return (
          <span className="px-2 py-0.5 rounded-full bg-elevated border border-line text-fg text-[10px] font-bold">
            Event
          </span>
        );
    }
  ...
  return (
    <div className="w-full max-w-7xl mx-auto p-3 sm:p-6 space-y-4 sm:space-y-6 overflow-hidden">
      ...
      <div className="bg-card border border-line rounded-2xl p-4 sm:p-5 shadow-sm dark:shadow-lg flex flex-col sm:flex-row sm:items-center justify-between gap-4 transition-colors">
        <div>
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          >
            ...
            <option value="ai_restructure" className="bg-card">AI Restructures</option>
            <option value="retroplan_shift" className="bg-card">Retroplan Shifts</option>
            <option value="status_change" className="bg-card">Status Changes</option>
            <option value="update" className="bg-card">Updates</option>
            <option value="create" className="bg-card">Creations</option>
          </select>
      ...
      {/* History Stream */}
      <div className="bg-card border border-line rounded-2xl p-5 shadow-sm dark:shadow-xl transition-colors">
          ...
          <div className="relative pl-6 space-y-6 before:absolute before:left-2.5 before:top-2 before:bottom-2 before:w-0.5 before:bg-elevated">
            {filteredHistory.map((entry) => (
              <div key={entry.id} className="relative group">
                ...
                <div className="bg-elevated/60 hover:bg-elevated p-4 rounded-xl border border-line transition-all space-y-2">
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
                    <div className="flex items-center gap-2">
                      ...
                      />
                      <span className="font-bold text-xs text-fg">{entry.userName}</span>
                  ...
                  {entry.diff && (
                    <div className="p-2.5 rounded-lg bg-elevated border border-line font-mono text-[11px] grid grid-cols-1 sm:grid-cols-2 gap-2 mt-2">
                      <div className="text-rose-600 dark:text-rose-400 flex items-center gap-1 truncate">
                        <span className="text-fg-muted">- Old ({entry.diff.field}):</span>
                      ...
                      <div className="text-emerald-600 dark:text-emerald-400 flex items-center gap-1 truncate">
                        <span className="text-fg-muted">+ New ({entry.diff.field}):</span>
